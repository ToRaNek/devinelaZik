...
/**
 * Fonction pour récupérer les détails d'une piste Spotify, y compris son ID
 * Cette fonction est utile pour compléter les questions avec des IDs de pistes
 */
async function getTrackDetails(trackId, userId) {
    try {
        console.log(`Récupération des détails de la piste ${trackId} pour l'utilisateur ${userId}`);
        const accessToken = await getValidSpotifyToken(userId);
        ...
        if (!response.ok) {
            console.error(`Échec lors de la récupération des détails de la piste: ${response.status} ${response.statusText}`);
            const errorBody = await response.text();
            console.error(`Réponse d'erreur: ${errorBody}`);
            throw new Error(`Échec lors de la récupération des détails de la piste: ${response.status} ${response.statusText}`);
        }

        const data = await response.json();
        console.log(`Détails de la piste ${trackId} récupérés avec succès`);
        return data;
    } catch (error) {
...
/**
 * Amélioration de la génération des questions pour inclure les IDs des pistes Spotify
 */
function enrichQuestionsWithSpotifyIds(questions, tracks) {
    return questions.map(question => {
        // Pour les questions avec prévisualisation mais sans ID de piste explicite
        if ((question.type === 'song' || question.type === 'artist') &&
            question.previewUrl && !question.spotifyTrackId) {

            // Chercher la piste correspondante dans la collection de pistes
            const matchingTrack = tracks.find(track => {
                // Correspondance par URL de prévisualisation si disponible
                if (track.preview_url === question.previewUrl) {
                    return true;
                }

                // Correspondance par titre et artiste
                if (question.type === 'song' &&
                    normalizeString(track.name) === normalizeString(question.answer) &&
                    track.artists.some(artist => normalizeString(artist.name) === normalizeString(question.artistName))) {
                    return true;
                }
            ...
            // Si une piste correspondante est trouvée, ajouter son ID
            if (matchingTrack) {
                return {
                    ...question,
                    spotifyTrackId: matchingTrack.id,
                    // Utiliser l'URL de prévisualisation officielle si disponible
                    previewUrl: matchingTrack.preview_url || question.previewUrl
                };
            }
        }
...
// Fonction utilitaire pour normaliser les chaînes pour la comparaison
...
// Ajoutez ces fonctions à vos exports
module.exports = {
