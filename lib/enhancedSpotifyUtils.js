...
function generateMultipleChoiceQuestions(tracks, artists, albums, count) {
    ...
    const generateWrongOptions = (correctOption, allOptions, count = 3) => {
        ...
        const artistFilter = question.type === 'song' && question.artistName ?
            // For song questions, filter options by same artist
            allOptions.filter(opt =>
                opt.artists.some(a => a.name.toLowerCase() === question.artistName.toLowerCase())
            ) :
            // For other question types, use normal filtering
            allOptions;
    ...
    const maxSongsAvailable = Math.min(songCount, allTracks.length);
    ...
    const selectedSongTracks = shuffleArray(allTracks).slice(0, finalSongCount);
