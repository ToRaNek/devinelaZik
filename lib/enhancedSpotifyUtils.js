...
function generateMultipleChoiceQuestions(tracks, artists, albums, count) {
    ...
    const remaining = count - (questionsPerType * 3);

    // Calcul du nombre exact de questions par type
    const countsPerType = {
        artist: questionsPerType + (remaining > 0 ? 1 : 0),
        song: questionsPerType + (remaining > 1 ? 1 : 0),
        album: questionsPerType + (remaining > 2 ? 1 : 0)
    };

    // 1. Questions sur les artistes
    if (artists && artists.length > 0) {
        const artistNames = artists.map(artist => artist.name);

        for (let i = 0; i < Math.min(countsPerType.artist, artists.length); i++) {
            const artist = artists[i];

            // Trouver une piste avec preview de cet artiste
    ...
    // 2. Questions sur les titres
    if (tracks && tracks.length > 0) {
        const trackNames = tracks.map(track => track.name);

        for (let i = 0; i < Math.min(countsPerType.song, tracks.length); i++) {
    ...
    if (albums && albums.length > 0) {
        ...
        for (let i = 0; i < Math.min(countsPerType.album, albums.length); i++) {
    ...
    // Si on n'a pas assez de questions, en générer d'autres
    while (questions.length < count && tracks.length > 0) {
        // Choisir un type de question aléatoire
        const type = questionTypes[Math.floor(Math.random() * questionTypes.length)];

        if (type === 'artist' && tracks.length > 0) {
            const track = tracks[Math.floor(Math.random() * tracks.length)];
            const artist = track.artists[0];
            if (track.preview_url) {
                const wrongOptions = generateWrongOptions(artist.name, artists.map(a => a.name));
        ...
        } else if (type === 'song' && tracks.length > 0) {
            const track = tracks[Math.floor(Math.random() * tracks.length)];
            if (track.preview_url) {
                const wrongOptions = generateWrongOptions(track.name, tracks.map(t => t.name));
            ...
            const wrongOptions = generateWrongOptions(album.name, tracks.map(t => t.album.name));

            questions.push({
                type: 'album',
                quizType: 'multiple_choice',
                question: `Quel est cet album de ${track.artists[0].name} ?`,
                answer: album.name,
                artistName: track.artists[0].name,
                albumCover: album.images[0]?.url,
                options: shuffleArray([album.name, ...wrongOptions])
    ...
    while (questions.length < count && tracks.length > 0) {
        ...
        if (type === 'artist' && tracks.length > 0) {
            ...
            questions.push({
                ...
                artistName: track.artists[0].name,
                albumCover: album.images[0]?.url,
                displayFormat: `${album.name} (${track.artists[0].name})`
            });
        }
    }
...
function getSampleQuestions(count, quizType = 'multiple_choice') {
    ...
    const filteredQuestions = sampleQuestions.filter(q => q.quizType === quizType);

    // Mélanger et sélectionner le nombre demandé
    const shuffled = [...filteredQuestions].sort(() => 0.5 - Math.random());
    const selected = shuffled.slice(0, Math.min(count, shuffled.length));

    // Mettre à jour les numéros de rounds
