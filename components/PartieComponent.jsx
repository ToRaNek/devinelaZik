...
export default function PartieComponent({ roomCode }) {
  ...
  useEffect(() => {
    if (!socket || !roomCode || !session) return;
    ...
    socket.on('playerLeft', (userId) => {
      ...
      // Trouver le joueur qui est parti
    ...
    socket.on('gameStarted', ({ rounds }) => {
      ...
      setMessages(prev => [...prev, {
        system: true,
        message: `La partie commence! ${rounds} rounds au total.`
      }]);
    });
    ...
    socket.on('newQuestion', (question) => {
      ...
      setMessages(prev => [...prev, {
        system: true,
        message: `Round ${round}/${totalRounds}: Nouvelle question! (${question.type})`
      }]);
    });
    ...
    socket.on('questionTimeout', ({ correctAnswer }) => {
      ...
      setMessages(prev => [...prev, {
        system: true,
        message: `Temps écoulé! La réponse était: ${correctAnswer}`
      }]);
    ...
    socket.on('roundEnd', ({ scores, nextRound }) => {
    ...
    socket.on('answerResult', ({ correct, points, answer }) => {
      ...
      setMessages(prev => [...prev, {
        ...
        message: correct ? 
          ...
          `Incorrect. La réponse était: ${answer}`
      }]);
  ...
  const submitAnswer = (e) => {
    e.preventDefault();
    if (!socket || !answer.trim() || !currentQuestion) return;
