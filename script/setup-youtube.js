...
console.log('📝 Création d\'un script de test...');
try {
    const testScriptPath = './scripts/test-audio.js';
    const testScriptContent = `// scripts/test-audio.js
const audioService = require('../lib/audioPreviewService');
...
async function runTests() {
  console.log('🎵 Test du service audio 🎵');
  
  let successes = 0;
  
  for (const test of testCases) {
    console.log(\`\\n🔍 Test avec: \${test.artist} - \${test.track}\`);
    
    try {
      const result = await audioService.getSongPreview(test.artist, test.track);
      
      if (result) {
        console.log(\`✅ Succès! URL trouvée: \${result.directAudioUrl ? 'URL directe disponible' : result.embedUrl ? 'URL embed disponible' : 'Aucune URL'}\`);
        console.log(\`📊 Détails: \${result.title} | Source: \${result.previewSource}\`);
        successes++;
      } else {
  ...
  console.log(\`\\n📈 Résultats: \${successes}/${testCases.length} tests réussis\`);
}
...
runTests().catch(console.error);`;
    ...
    // Ajouter script au package.json
    try {
        const packageJsonPath = './package.json';
        if (fs.existsSync(packageJsonPath)) {
            const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf8'));
            if (!packageJson.scripts['test-audio']) {
                packageJson.scripts['test-audio'] = 'node scripts/test-audio.js';
                fs.writeFileSync(packageJsonPath, JSON.stringify(packageJson, null, 2));
                console.log('✅ Script de test ajouté à package.json');
            }
        }
    } catch (error) {
...
Pour commencer à utiliser le nouveau service:
...
3. Intégrer dans votre projet en remplaçant l'ancien service dans:
   - lib/youtubeUtils.js
